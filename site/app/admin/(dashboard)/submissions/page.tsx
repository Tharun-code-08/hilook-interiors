  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.75rem" }}>
        <h1 style={{ fontFamily: "Georgia, serif", fontSize: "1.6rem", color: "#26231F" }}>Contact Inbox</h1>
        <a
        ...
        <div style={{ display: "grid", gap: "0.75rem" }}>
          {submissions.map((s) => (
            ...
            >
              ...
              <p style={{ margin: "0.9rem 0", color: "#5E5951", fontSize: "0.9rem", lineHeight: 1.6 }}>{s.message}</p>
              <div style={{ display: "flex", gap: "0.6rem", flexWrap: "wrap" }}>
                <button
                  onClick={() => patch(s.id, { read: !s.read })}
                  style={{ fontSize: "0.72rem", padding: "0.35rem 0.75rem", border: "1px solid rgba(74,63,51,0.24)", background: "transparent" }}
                >
                  {s.read ? "Mark unread" : "Mark read"}
                </button>
                <button
                  onClick={() => patch(s.id, { responded: !s.responded })}
                  style={{ fontSize: "0.72rem", padding: "0.35rem 0.75rem", border: "1px solid rgba(74,63,51,0.24)", background: s.responded ? "#173F35" : "transparent", color: s.responded ? "#fff" : "#26231F" }}
                >
                  {s.responded ? "Responded" : "Mark responded"}
                </button>
                <button
                  onClick={() => remove(s.id)}
                  style={{ fontSize: "0.72rem", padding: "0.35rem 0.75rem", border: "1px solid #5A2630", color: "#5A2630", background: "transparent" }}
                >
                  Delete
                </button>
              </div>
